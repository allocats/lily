...
AstNodeId parse_struct_decl(Parser* p) {
    AstNodeId id  = parser_create_node(p, AST_STRUCT, AST_FLAGS_IS_TOP_LEVEL);
    ...
    node -> as.struct_decl.name_id = STRING_INTERNER_LOOKUP_TOKEN(name);
    ...
    while (!parser_check(p, TOK_RBRACE)) {
        ...
        if (UNLIKELY(node -> as.struct_decl.field_count >= node -> as.struct_decl.field_capacity)) {
            ...
            debug_printf(
                "Struct(%.*s) fields realloc from %ld -> %ld bytes\n",
                name -> lexeme.length,
                name -> lexeme.pointer,
                size,
                size * 2
