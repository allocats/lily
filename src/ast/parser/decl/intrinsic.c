...
AstNodeId parse_intrinsic_decl(Parser* p, StringId name) {
    if (!parser_check(p, TOK_KW_FN)) {
        ...
        diagnostic_add_token(
            ...
            DIAG_LOC_WHOLE_TOK,
            "expected 'fn'",
            "insert fn for intrinsic declaration"
        );
    ...
    AstNodeId id = parser_create_node(p, AST_FUNCTION_DECL, AST_FLAGS_IS_TOP_DECL | AST_FLAGS_IS_INTRINSIC, -4);
